...
class ScapyNetwork(threading.Thread):
        ...
        if DHCP in incoming:
            ...
            if incoming[UDP].dport == 67:
                if incoming[BOOTP].op == 1:
                    req_type = next(
                        opt[1]
                        for opt in incoming[DHCP].options
            ...
            if incoming[ICMP].type == 8:
                ...
                eth_ip_header = Ether(src=self.bss.mac, dst=incoming[Ether].src) / IP(
                    dst=incoming[IP].src, src=sender_ip
                )
                icmp = None
                if incoming[IP].dst not in m:
                    ...
                    icmp = (
                        ICMP(type=0, seq=incoming[ICMP].seq, id=incoming[ICMP].id)
                        / incoming[ICMP].load
                    )
            ...
            eth_ip_header = Ether(src=self.bss.mac, dst=incoming[Ether].src) / IP(
                dst=incoming[IP].src, src=self.ip
            )
    ...
    def reply_dhcp_offer(self, incoming):
        ...
        packet = (
            Ether(dst="ff:ff:ff:ff:ff:ff", src=self.bss.mac, type=0x800)
